...
public class Cycle {
	...
	/**
	 * Check for duplicate nodes inside the cycle
	 * we don't want any cycles that have start-node inside
	 */
	public boolean isValid() {
		for (int i = 1; i<= this.maximum; i++)
			for (int j = 1; j<= this.maximum; j++)
				if (members[i] == members[j] && j != i) return false;
		return true;
	}
	...
	public boolean push(int v) {
		if (v == members[1] && current == maximum && isValid()) return true;
	...
	/** Create all variants of the given cycle
	 *	(1 2 3) returns:
	 *			(2 3 1)
	 *			(3 1 2)
	 *			(1 2 3)
	...
	/** Create one variant of the given cycle
	 *  (1 2 3 4) => (2 3 4 1)
	 */
	public void perm() {
